-- v53 consolidated post-draft setup. Run this instead of 0012 + 0013 if neither has been run.
...
UPDATE players SET name='Stephen Scalfani' WHERE name='Stephen Sclafani' COLLATE NOCASE
  AND NOT EXISTS (SELECT 1 FROM players WHERE name='Stephen Scalfani' COLLATE NOCASE);
...
INSERT OR IGNORE INTO players(player_id,name,class_year,retired) VALUES('stephen_scalfani','Stephen Scalfani',NULL,0);
...
INSERT INTO team_rosters(season,player_id,team_id,role,assigned_by) SELECT 2026,player_id,'underdawgs','player','post-draft-v50' FROM players WHERE name='Stephen Scalfani' COLLATE NOCASE;
...
-- Apply commissioner class-year clarifications after roster creation/reset.

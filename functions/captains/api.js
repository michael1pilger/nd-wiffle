...
async function accessIdentity(context){
  // New Workers/Access integration exposes a verified identity directly.
  try{
  ...
  // Traditional self-hosted Access applications forward these to the origin.
  ...
  // Do not trust/decode a browser-supplied CF_Authorization cookie as identity
  // unless Access has also supplied a verified origin identity/header.
...
export async function onRequestGet(context){
  ...
  return json({ok:true,build:"v77",season,captain,availability:availability.results||[],scheduled:scheduled.results||[]});
}
export async function onRequestPost(context){
  ...
  for(const w of windows){
    ...
    const d=new Date(date+"T12:00:00Z"),weekend=d.getUTCDay()===0||d.getUTCDay()===6,s=toMin(start),e=toMin(end),first=weekend?11*60:15*60+30,last=18*60+30;
    if(s<first||s>last||s%15!==0||e-s!==90)continue;

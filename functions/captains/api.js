...
export async function onRequestGet(context) {
  ...
  const [teams, availability, scheduled, completed] = await DB.batch([
    ...
    DB.prepare(`
      ...
      JOIN teams t ON t.team_id = ca.team_id
      WHERE ca.season = ?
        AND ca.availability_date BETWEEN '2026-09-15' AND '2026-10-17'
...
export async function onRequestPost(context) {
  ...
  for (const w of windows) {
    ...
    if (date < "2026-09-15" || date > "2026-10-17") continue;
  ...
  const scheduled = await DB.prepare(`
    ...
    WHERE ss.season = ?
      ...
      AND ss.series_date BETWEEN '2026-09-15' AND '2026-10-17'
  `).bind(season).all();
  ...
  await DB.prepare(`
    ...
    WHERE season = ?
      AND team_id = ?
      AND availability_date BETWEEN '2026-09-15' AND '2026-10-17'

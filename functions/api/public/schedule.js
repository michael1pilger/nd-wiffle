...
export async function onRequestGet(context){
 ...
 try{
  ...
  `).bind(season).all();
  return json({ok:true,build:"v64",season,scheduled:rows.results||[]});
 }catch(err){
  const msg=String(err?.message||err);
  if(msg.includes("no such table"))return json({ok:true,build:"v64",season,scheduled:[],schema_ready:false});

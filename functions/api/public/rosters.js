  ...
  try{
    const rows=await DB.prepare(`
      ...
      ORDER BY t.display_name,
        ...
        p.name
    `).bind(season).all();
    return json({ok:true,build:"v80",season,rosters:rows.results||[]});
  }catch(err){
    const msg=String(err?.message||err);
    if(msg.includes("no such table"))return json({ok:true,build:"v80",season,rosters:[],schema_ready:false});

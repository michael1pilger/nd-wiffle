...
export async function onRequestGet(context){const DB=context.env.DB;if(!DB)return json({ok:false,error:"D1 binding DB is missing."},500);if(!(await ready(DB)))return json({ok:false,code:"DRAFT_SCHEMA_MISSING",error:"Run migrations/0005_draft_2026.sql on this D1 database."},503);if(!(await rosterReady(DB)))return json({ok:false,code:"TEAM_ROSTERS_SCHEMA_MISSING",error:"The team_rosters table is missing. Run migrations/0008_ensure_team_rosters.sql on this D1 database."},503);const season=Number(new URL(context.request.url).searchParams.get("season")||2026);try{return json({ok:true,build:"v47",season,actor_email:context.data.actorEmail||null,...await state(DB,season)})}catch(e){return json({ok:false,error:"Draft query failed.",detail:String(e?.message||e)},500)}}
export async function onRequestPost(context){const DB=context.env.DB;if(!DB)return json({ok:false,error:"D1 binding DB is missing."},500);if(!(await ready(DB)))return json({ok:false,code:"DRAFT_SCHEMA_MISSING",error:"Run migrations/0005_draft_2026.sql on this D1 database."},503);if(!(await rosterReady(DB)))return json({ok:false,code:"TEAM_ROSTERS_SCHEMA_MISSING",error:"The team_rosters table is missing. Run migrations/0008_ensure_team_rosters.sql on this D1 database."},503);let b;try{b=await context.request.json()}catch{return json({ok:false,error:"Invalid JSON body."},400)}const season=Number(b.season||2026),actor=context.data.actorEmail||"unknown-access-user";try{
